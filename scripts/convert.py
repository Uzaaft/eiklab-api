...
class Params:
    ...
    def load(model_plus: "ModelPlus") -> "Params":
        model_plus.paths[0].parent / "params.json"
        hf_transformer_config_path = model_plus.paths[0].parent / "config.json"

        if hf_transformer_config_path.exists():
            params = Params.loadHFTransformerJson(
                model_plus.model, hf_transformer_config_path
            )
        else:
            params = Params.guessed(model_plus.model)

        print(
            f"params: n_vocab:{params.n_vocab} n_embd:{params.n_embd} n_mult:{params.n_mult} n_head:{params.n_head} n_layer:{params.n_layer}"
        )
        return params
...
def dequantize_q4(
    ...
    qvalues = np.zeros(
        [qvalues_pack8.shape[0], qvalues_pack8.shape[1] * 2], dtype=np.uint8
    )
...
class GGMLQuantizedTensor(Tensor):
    ...
    def __init__(self, ndarray: NDArray, shape: List[int], data_type: DataType) -> None:
        ...
        assert isinstance(
            data_type, QuantizedDataType
        )  # redundant, but mypy complains without this
    ...
    def permute(self, n_head: int) -> "GGMLQuantizedTensor":
        return GGMLQuantizedTensor(
            permute(self.ndarray, n_head), self.shape, self.data_type
        )
...
class GPTQForLLaMaQuantizedTensor(Tensor):
    ...
    def astype(self, data_type: DataType) -> Tensor:
        if isinstance(data_type, QuantizedDataType):
            assert (
                self.g_idx is None
                and data_type.have_addends is True
                and data_type.have_g_idx is False
            )
            return self.regroup(data_type.groupsize)

        dequantized = dequantize_q4(
            np.ascontiguousarray(self.qweight), self.scales, self.addends, self.g_idx
        )
    ...
    def regroup(self, new_groupsize: int = 32) -> "GPTQForLLaMaQuantizedTensor":
        ...
        assert (
            old_groupsize >= new_groupsize and old_groupsize % new_groupsize == 0
        ), old_groupsize
        ret = copy.copy(self)
        ...
        ret.data_type = QuantizedDataType(
            groupsize=new_groupsize, have_addends=True, have_g_idx=False
        )
        return ret
    ...
    def to_ggml(self) -> GGMLQuantizedTensor:
        ...
        grouped = np.concatenate(
            [scales_view, addends_view, grouped], axis=2, casting="no"
        )
    ...
    def validate_conversion_to(self, data_type: DataType) -> None:
        ...
        if isinstance(data_type, QuantizedDataType):
            ...
            if self.data_type.have_g_idx:
                ...
                sys.exit(1)
            assert (
                not data_type.have_g_idx
                and self.data_type.have_addends
                and data_type.have_addends
            )
...
def handle_quantization(model: LazyModel) -> LazyModel:
    ...
    def convert(name: str) -> Tuple[str, LazyTensor]:
        if name.endswith(".qweight"):
            ...
            data_type = QuantizedDataType(
                groupsize=groupsize, have_addends=True, have_g_idx=have_g_idx
            )
...
def bounded_parallel_map(
    func: Callable[[In], Out], iterable: Iterable[In], concurrency: int
) -> Iterable[Out]:
...
class OutputFile:
    ...
    def write_tensor_header(
        ...
        self.fout.write(
            struct.pack("iii", len(shape), len(sname), DATA_TYPE_TO_FTYPE[data_type])
        )
...
def pick_output_type(model: LazyModel, output_type_str: Optional[str]) -> GGMLFileType:
    ...
    if output_type_str == "q4_1" or (
        output_type_str is None
        and isinstance(wq_type, QuantizedDataType)
